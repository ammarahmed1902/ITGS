...
import { LogOut, Plus, Edit, Trash2, Search, Download, Users, Contact } from 'lucide-react';
...
const AdminPage = ({
  ...
  // Reset editor view if the current role lacks permission (must run before conditional returns)
  React.useEffect(() => {
    if (!currentUser) return;
    if (view === 'blog-editor' && !allowedSections.includes('Blog')) {
      setView('dashboard');
      setEditingPost(null);
    }
    if (view === 'job-editor' && !allowedSections.includes('Jobs')) {
      setView('dashboard');
      setEditingJob(null);
    }
  }, [view, allowedSections, currentUser]);
  ...
  return (
    <>
      <PageMeta title="Admin Dashboard" noIndex path={ROUTES.admin} />
            ...
            <button
              ...
              className={`px-6 py-3 rounded-full text-sm font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${
                adminSection === section
                  ...
                  : 'bg-midnight/5 border border-midnight/10 text-steel hover:text-electric'
              }`}
            >
              {section === 'Team' && <Contact size={15} />}
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {filteredPosts.length > 0 ? filteredPosts.map(post => (
                <div key={post.id} className="bg-white p-6 rounded-2xl border border-midnight/5 flex items-center justify-between group hover:border-electric/30 transition-all">
                  ...
                  <div className="flex items-center gap-2 md:opacity-70 md:group-hover:opacity-100 transition-opacity">
                ...
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <select
                      ...
                      className="bg-starfield border border-midnight/10 rounded-xl px-4 py-2.5 text-sm font-bold text-midnight focus:border-electric outline-none transition-all"
                      aria-label={`Update status for ${app.firstName} ${app.lastName}`}
                    >
                      {APPLICATION_STATUSES.filter((s): s is ApplicationStatus => s !== 'All').map((status) => (
                        <option key={status} value={status}>{status}</option>
                      ))}
                    </select>
                    <button
                      onClick={() => window.confirm(`Remove application from ${app.firstName} ${app.lastName}?`) && onDeleteApplication(app.id)}
                      className="px-4 py-2.5 bg-red-50 text-red-600 font-bold rounded-xl text-sm"
                    >
                      Remove
                    </button>
                  </div>
